...
gulp.task('build',
  gulp.series(clean, pages, sass, images, inlineNoMinify));
...
function pages() {
    return gulp.src(['src/pages/**/*.ftl', '!src/pages/archive/**/*.ftl', 'src/pages/**/*.html'])
    .pipe(panini({
      root: 'src/pages',
      layouts: 'src/layouts',
...
function inline() {
  return gulp.src('dist/**/*.ftl')
    .pipe($.if(PRODUCTION, inliner('dist/css/app.css')))
    .pipe(gulp.dest('dist'));
}

// Inline CSS and not minify HTML
function inlineNoMinify() {
    return gulp.src('dist/**/*.ftl')
        .pipe($.if(PRODUCTION, inliner('dist/css/app.css')))
        .pipe(gulp.dest('dist'));
}
    ...
    middleware: [
      {
        ...
        handle: function (req, res, next) {
          ...
          const filePath = `${url_parts.query['filePath']}`;
          const fileData = `<html>${fs.readFileSync(filePath)}</html>`;
          res.end(fileData);
        }
      }
      ]
...
function watch() {
  gulp.watch('src/pages/**/*.ftl').on('all', gulp.series(pages, inline, browser.reload));
